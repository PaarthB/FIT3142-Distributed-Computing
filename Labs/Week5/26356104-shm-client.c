...
int main()
{
	 ...
	 */
        ...
        mydata = shm;
        fprintf(stdout, "Present value: %d\n", mydata->present);
        while (mydata->present >= 15)
        {
            fprintf(stdout, "Already 4 clients accessing the server. Please wait for one to leave.\n");
            sleep(20);
        }
        mydata->start = 1;
        if (mydata->present == 0)
           mydata->present += CLIENT_1;
        else if (mydata->present == 1)
           mydata->present += CLIENT_2;
        else if (mydata->present == 3)
           mydata->present += CLIENT_3;
        else if (mydata->present == 7)
